...
class DelayedRewardWrapper(gym.Wrapper):
    """Wrapper that delivers a sparse/delayed reward only at the end of the episode.
    
    All step rewards are 0, except for the last step which returns the sum of
    all rewards accumulated during the episode.
    """
    ...
    def step(self, action):
        ...
        # If episode is done, return accumulated reward, otherwise return 0
        modified_reward = self.episode_rewards if (terminated or truncated) else 0
        
        # Reset accumulator if episode is done
        if (terminated or truncated):
            self.episode_rewards = 0
        
        return obs, modified_reward, terminated, truncated, infos
...
def run_environment(outfile, env_name, timesteps, n_envs):
    ...
    env = make_vec_env(
        ...
        wrapper_class=DelayedRewardWrapper
    )
